...
class WithingsStatusNotZeroError(Exception):
    """
    Exception raised when a Withings API response has a status of 200
    but the Withings status is not 0.
    """
    def __init__(self, data: dict, url: str, dct: dict):
        ...
        Args:
            ...
            dct (dict): The dictionary containing the response details,
                including status code and error message.
        """
        self.message = (
            f"The post request response status is 200, "
            f"i.e. the post request was successful, "
...
class InvalidDataFieldWarning(Warning):
    def __init__(self, fct_name, data_field):
        ...
        message = (f"{data_field} is not a valid data field for "
                   f"{fct_name.removeprefix("data_")}"
                   f"and will not be used in the request.")
