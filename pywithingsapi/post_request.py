...
def get_data_dict(data: dict, url: str, user: WithingsUser, to_json: bool = False) -> dict:
    ...
    This function sends a POST request to the given URL with the specified data
        and user authentication headers.
    If the `to_json` flag is set to True, the response is saved as a JSON file
        in the user's directory.

    Args:
        data (dict): The data to be sent in the POST request.
        url (str): The URL to send the POST request to.
        user (WithingsUser): An instance of `WithingsUser` which is necessary
    ...
    if to_json:
        ...
        try:
            ...
            print(f"An error occurred while accessing the directory or writing the file "
                  f"({e}, {type(e).__name__}).")
